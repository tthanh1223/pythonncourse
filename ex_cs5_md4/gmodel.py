...
def parse_email_body(raw_body):
    try:
        # Decompress the body first if it's compressed
        decompressed_body = zlib.decompress(raw_body)

        # Create an email message object from the raw bytes
        message = email.message_from_bytes(decompressed_body)
        ...
        if message.is_multipart():
            ...
            # If not multipart, handle the single-part message
...
for message_row in cur_1 :
    ...
    # Parse and handle the email body
    parsed_body = parse_email_body(message_row[1])

    if parsed_body is None:
        print(f"Skipping message due to body parsing error: {guid}")
        continue
    ...
    cur.execute(
        ...
        (guid, sender_id, subject_id, sent_at,
         zlib.compress(message_row[0].encode()), zlib.compress(parsed_body.encode())))
